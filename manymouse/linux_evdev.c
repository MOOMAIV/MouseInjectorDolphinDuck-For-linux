...
/*
 * Exclusively grab (or release) every open mouse fd that ManyMouse itself
 * is reading from.
 *
 * IMPORTANT: EVIOCGRAB is per FILE DESCRIPTOR, not per device. If a
 * separate fd is opened to the same /dev/input/eventN node and grabbed
 * there, the kernel routes all further input events ONLY to that
 * grabbing fd — every other open fd to that device, including
 * ManyMouse's own, goes silent. Grabbing must happen on these exact
 * fds so ManyMouse keeps receiving motion events while the desktop
 * compositor/cursor stops seeing them.
 */
int ManyMouse_LinuxEvdevGrab(int grab)
{
    ...
    for (i = 0; i < available_mice; i++)
    {
        if (mice[i].fd != -1)
        {
            if (ioctl(mice[i].fd, EVIOCGRAB, (void *) (long) grab) == 0)
                grabbed_count++;
        }
    }
    return grabbed_count;
} /* ManyMouse_LinuxEvdevGrab */
...
int ManyMouse_LinuxEvdevGrab(int grab) { (void)grab; return 0; }
